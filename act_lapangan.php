...
if ($doc_size > 100097152) {
	...
	header("location:add_lapangan.php?pesan=size");
}else{
	...
	if ($doc != "") {
		...
		if(in_array($ekstensi, $ekstensi_izin) === true)  {
            ...
            $query = mysqli_query($conn, "INSERT INTO lapangan VALUES ('','$tgl', '$kegiatan', '$jenis', '$doc_new')");

            // Mengecek apakah data gagal diinput atau tidak
            if($query){
            	header("location:add_lapangan.php?pesan=berhasil");
            } else {
                header("location:add_lapangan.php?pesan=gagal");
            }

        } else { 
        	// Jika ekstensinya tidak sesuai dengan apa yg kita tetapkan maka error
        	header("location:add_lapangan.php?pesan=ekstensi");        }

    }else{

    	// Apabila tidak ada file yang diupload maka akan menjalankan code dibawah ini
    	 $query = mysqli_query($conn, "INSERT INTO lapangan (tgl, kegiatan, jenis) VALUES ('$tgl', '$kegiatan', '$jenis')");
