...
if (isset($_POST['id'])) {
    ...
    if ($doc_size > 100097152) {
       ...
       header("location:lapangan.php?pesan=size");
       ...
       if ($doc != "") {
          ...
          if(in_array($ekstensi, $ekstensi_izin) === true)  {
            ...
            $get_doc = "SELECT doc FROM lapangan WHERE id='$id'";
            ...
            $query = mysqli_query($conn, "UPDATE lapangan SET tgl='$tgl', kegiatan='$kegiatan', jenis='$jenis', doc='$doc_new' WHERE id='$id'");

            // Mengecek apakah data gagal diinput atau tidak
            if($query){
            	header("location:lapangan.php?pesan=berhasil");
            } else {
                header("location:lapangan.php?pesan=gagal");
            }

        } else { 
        	// Jika ekstensinya tidak sesuai dengan apa yg kita tetapkan maka error
        	header("location:lapangan.php?pesan=ekstensi");        }

        }else{

    	// Apabila tidak ada file yang diupload maka akan menjalankan code dibawah ini
          $query = mysqli_query($conn, "UPDATE lapangan SET tgl='$tgl', kegiatan='$kegiatan', jenis='$jenis' WHERE id='$id'");
